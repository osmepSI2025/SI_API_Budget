...
{
    public class RecP301Service : IRecP301Service
    {
        private readonly IRecPRsRepository _repository;
        private readonly IApiInformationRepository _repositoryApi;
        private readonly ICallAPIService _serviceApi;

        public RecP301Service(IRecPRsRepository repository, IApiInformationRepository repositoryApi, ICallAPIService serviceApi)
        {
        ...
        public async Task<ApiRecP301ResponseModel> SendDataAsync(RecP301Models Senddata)
        {
            try
            {
                var LApi = await _repositoryApi.GetAllAsync(new MapiInformationModels { ServiceNameCode = "Rec_P_301" });
                if (!LApi.Any())
                {
                    return null;
                }
                ...
                var resultApi = await _serviceApi.RecDataApiAsync_RecP301(apiParam,Senddata);
